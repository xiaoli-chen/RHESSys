...
double	compute_N_leached(int verbose_flag, 
	...
	if (Qout > ZERO) {
	    ...
	    if (N_decay_rate > 0.0) {	
		    ...
		    Qout_navail = total_nitrate
			    / (1.0 - exp(-1.0 * N_decay_rate * z2_N) )
			    * (exp(-1.0 * N_decay_rate * Qout_z1)
			    - exp(-1.0 * N_decay_rate * (z2)));
	    }
	    else {
	    ...
	    /*------------------------------------------------------*/
		  ...
		  //// 
		  if (nabsorbed > Qout_navail) {
				  Qout_navail=0;
		  }
		  else Qout_navail = Qout_navail-nabsorbed;

		  if (available_water > ZERO) {
			  Qout_nleached = Qout_navail * Qout/available_water;	
		  }
		  else Qout_nleached = 0.0;
	...
	if (Qout_nleached > Qout_navail) Qout_nleached = Qout_navail;
	/*------------------------------------------------------*/
	...
	nleached = max(nleached, Qout_nleached);
